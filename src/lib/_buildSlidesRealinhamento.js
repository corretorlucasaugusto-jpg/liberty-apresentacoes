...
export function buildSlidesRealinhamento(d, slides = []) {
  ...
  var sh = function(lbl, lblColor, title, sub) {
    return '<div style="padding:40px 64px 22px;border-bottom:1px solid '+BL+';display:flex;justify-content:space-between;align-items:flex-end;flex-shrink:0">'
      +'<div style="display:flex;flex-direction:column;gap:7px">'
        +'<div style="font-size:.62rem;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:'+(lblColor||GOLD)+'">'+lbl+'</div>'
        +'<div style="font-size:clamp(1.8rem,3vw,2.8rem);font-weight:700;color:'+DARK+';line-height:1;letter-spacing:-.03em">'+title+'</div>'
      +'</div>'
  ...
  // ══════════════════════════════════════════════════════
  // S1 — CAPA (totalmente clara, azul claro no lado direito)
  // ══════════════════════════════════════════════════════
  var kpis = [
  ...
  slides.push(slide('s1r',
    ...
    +'<div style="padding:56px 64px;display:flex;flex-direction:column;justify-content:space-between;border-right:1px solid '+BL+'">'
      +'<div style="display:flex;align-items:center;gap:12px">'+LOGO
        +'<span style="font-size:.56rem;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:'+LGRAY+'">Brasília · DF</span>'
      +'</div>'
      +'<div>'
        +'<div style="font-size:.58rem;color:'+LGRAY+';letter-spacing:.16em;text-transform:uppercase;font-weight:500;margin-bottom:14px">Liberty Imóveis</div>'
        +'<div style="font-size:clamp(2.4rem,4vw,4.2rem);font-weight:700;color:'+DARK+';line-height:.95;letter-spacing:-.04em;margin-bottom:18px">Alinhamento<br><span style="color:'+GOLD+'">Estratégico</span></div>'
        +'<div style="font-size:.84rem;color:'+GRAY+';line-height:1.65">'+e(d.residencial)+(d.bairro?' · '+e(d.bairro):'')+'</div>'
      +'</div>'
      +'<div style="font-size:.56rem;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:'+LGRAY+'">Preparado para '+e(d.nome)+' · '+new Date().toLocaleDateString('pt-BR')+'</div>'
    +'</div>'

    // Direita — azul claro
    +'<div style="background:'+BLLT+';padding:56px 64px;display:flex;flex-direction:column;justify-content:space-between">'
      +'<div style="font-size:.56rem;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:'+BLUE+'">Resumo</div>'
      +'<div style="display:flex;flex-direction:column;gap:1px;background:rgba(26,76,139,.12)">'
        +kpis.map(function(k){
          return '<div style="background:'+W+';padding:20px 24px;display:flex;justify-content:space-between;align-items:center">'
            +'<div style="font-size:.76rem;color:'+GRAY+'">'+k[0]+'</div>'
            +'<div style="font-size:1.02rem;font-weight:700;color:'+DARK+'">'+e(String(k[1]))+'</div>'
          +'</div>';
        }).join('')
        +(precoOrigNum>0
          ?'<div style="background:'+BLUE+';padding:20px 24px;display:flex;justify-content:space-between;align-items:center">'
            +'<div style="font-size:.76rem;color:rgba(255,255,255,.65)">Preço anunciado</div>'
            +'<div style="font-size:1.02rem;font-weight:700;color:'+W+'">'+fmtBRL(precoOrigNum)+'</div>'
          +'</div>'
          :'')
        +(maiorProposta>0
          ?'<div style="background:#fff8f0;border-top:2px solid '+GOLD+';padding:20px 24px;display:flex;justify-content:space-between;align-items:center">'
            +'<div style="font-size:.76rem;color:'+GRAY+'">Melhor proposta recebida</div>'
            +'<div style="font-size:1.02rem;font-weight:700;color:#92400e">'+fmtBRL(maiorProposta)+'</div>'
          +'</div>'
          :'')
      +'</div>'
      +'<div style="font-size:.52rem;color:'+LGRAY+';letter-spacing:.08em">Alinhamento Estratégico · '+new Date().toLocaleDateString('pt-BR')+'</div>'
    +'</div>'
    +'</div>'
  ));
  ...
  // ══════════════════════════════════════════════════════
  // S2 — LINHA DO TEMPO HORIZONTAL (responsiva)
  // ══════════════════════════════════════════════════════
  var events = [];
  ...
  (d.visitas||[]).forEach(function(v){
    if(v.data||v.qtd||v.feedback) events.push({
      ...
      sub:v.feedback?e(v.feedback.slice(0,55))+(v.feedback.length>55?'…':''):'',
      cor:BLUE
    });
  });
  ...
  // Tamanho responsivo baseado na quantidade
  var n = events.length;
  var icoSize = n<=4?48:n<=6?40:n<=8?34:28;
  var icoFont = n<=4?'1rem':n<=6?'.88rem':n<=8?'.78rem':'.65rem';
  var titleFs = n<=4?'.82rem':n<=6?'.74rem':n<=8?'.66rem':'.58rem';
  var subFs   = n<=4?'.68rem':n<=6?'.62rem':'.56rem';
  var dtFs    = n<=4?'.6rem':n<=6?'.54rem':'.5rem';
  var minW    = n<=4?'160px':n<=6?'130px':n<=8?'110px':'90px';

  var evHtml = events.map(function(ev,i){
    var isLast = i===events.length-1;
    return '<div style="display:flex;flex-direction:column;align-items:center;flex:1;min-width:'+minW+';max-width:220px;position:relative">'
      +(i>0?'<div style="position:absolute;top:'+(icoSize/2)+'px;right:50%;width:50%;height:2px;background:'+BL+'"></div>':'')
      +(!isLast?'<div style="position:absolute;top:'+(icoSize/2)+'px;left:50%;width:50%;height:2px;background:'+BL+'"></div>':'')
      +'<div style="width:'+icoSize+'px;height:'+icoSize+'px;border-radius:50%;background:'+W+';border:2px solid '+ev.cor+';display:flex;align-items:center;justify-content:center;font-size:'+icoFont+';position:relative;z-index:1;flex-shrink:0">'+ev.icon+'</div>'
      +(ev.iso?'<div style="font-size:'+dtFs+';font-weight:600;letter-spacing:.06em;text-transform:uppercase;color:'+LGRAY+';margin-top:8px;text-align:center">'+fmtData(ev.iso)+'</div>':'<div style="margin-top:8px;height:14px"></div>')
      +'<div style="font-size:'+titleFs+';font-weight:700;color:'+DARK+';margin-top:3px;text-align:center;line-height:1.3;padding:0 4px">'+ev.title+'</div>'
      +(ev.sub?'<div style="font-size:'+subFs+';color:'+GRAY+';margin-top:3px;text-align:center;line-height:1.35;font-style:italic;padding:0 4px">"'+ev.sub+'"</div>':'')
    +'</div>';
  }).join('');
  ...
  slides.push(slide('s2r',
    ...
    +'<div style="flex:1;display:flex;align-items:center;padding:0 48px">'
      +'<div style="display:flex;align-items:flex-start;width:100%;gap:0;padding:8px 0">'
        +evHtml
      +'</div>'
    +'</div>'
  ));
  ...
  // ══════════════════════════════════════════════════════
  // S3 — AÇÕES REALIZADAS (proporcional, sem lacunas)
  // ══════════════════════════════════════════════════════
  ...
  // Layout proporcional: 1 col se <=3, 2 col se <=8, 3 col se >8
  var na = acoesFilt.length;
  var nCols = na<=3?1:na<=8?2:3;
  // Padding por item proporcional ao espaço disponível
  var itemPad = na<=3?'26px 24px':na<=6?'20px 22px':na<=8?'16px 20px':'12px 18px';
  var icoBox  = na<=3?'52px':na<=6?'44px':'36px';
  var titleFsA= na<=3?'.88rem':na<=6?'.82rem':'.76rem';
  var descFsA = na<=3?'.74rem':na<=6?'.7rem':'.65rem';

  var acaoCards = acoesFilt.map(function(acao) {
    var desc = (d.acoesDesc && d.acoesDesc[acao]) || '';
    return '<div style="background:'+OFF+';padding:'+itemPad+';display:flex;gap:14px;align-items:flex-start">'
      +'<div style="width:'+icoBox+';height:'+icoBox+';min-width:'+icoBox+';background:'+BLUE+'12;border:1.5px solid '+BLUE+'30;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:'+BLUE+'">'+getIcon(acao)+'</div>'
      +'<div style="flex:1;min-width:0">'
        +'<div style="font-size:'+titleFsA+';font-weight:700;color:'+DARK+';margin-bottom:'+(desc?'4px':'0')+'">'+e(acao)+'</div>'
  ...
  slides.push(slide('s3r',
    ...
    +'<div style="flex:1;overflow-y:auto;padding:20px 64px 40px">'
      +'<div style="display:grid;grid-template-columns:repeat('+nCols+',1fr);gap:1px;background:'+BORD+';height:100%">'
        +acaoCards
      +'</div>'
    +'</div>'
  ));
  ...
  slides.push(slide('s4r',
    ...
    +sb(
      ...
      +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">'
        +(posMsg
          ?'<div style="background:'+GBG+';border-left:3px solid '+GOLD+';padding:16px 18px">'
            +'<div style="font-size:.62rem;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:'+GOLD+';margin-bottom:7px">Posicionamento</div>'
            +'<div style="font-size:.78rem;color:'+DARK+';line-height:1.65">'+posMsg+'</div>'
          +'</div>'
          :'<div></div>')
  ...
  // ══════════════════════════════════════════════════════
  // S5 — COMPARATIVO DE MERCADO (fontes maiores)
  // ══════════════════════════════════════════════════════
  (function(){
    ...
    var sec=function(cor,txt){
      return '<div style="font-size:.64rem;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:'+cor+';padding:12px 16px 6px;display:flex;align-items:center;gap:7px">'
        +'<span style="width:6px;height:6px;border-radius:50%;background:'+cor+'"></span>'+txt
      +'</div>';
    };
        ...
        +[
          ...
          {title:maiorProposta>0&&precoOrigNum>0?'A proposta está próxima':'O interesse existe',
            desc:maiorProposta>0&&precoOrigNum>0
              ?'A diferença entre o preço pedido e a melhor proposta é de '+fmtBRL(precoOrigNum-maiorProposta)+'. O custo de esperar pode superar essa diferença.'
              :'As visitas aconteceram. Há demanda real — o ajuste desbloquearia o fechamento.'},
        ].map(function(item){
          return '<div style="background:'+W+';padding:22px 24px">'
            +'<div style="font-size:.82rem;font-weight:700;color:'+DARK+';margin-bottom:5px">'+item.title+'</div>'
            +'<div style="font-size:.74rem;color:'+GRAY+';line-height:1.6">'+item.desc+'</div>'
          +'</div>';
        }).join('')
      +'</div>'
      +'</div>'
  ...
  // ══════════════════════════════════════════════════════
  // S7 — FAIXA DE PREÇO (sem $ duplicado)
  // ══════════════════════════════════════════════════════
  var prec=d.prec||null, temPrec=prec&&prec.mercado&&prec.mercado.total;

  // Fix: garantir que totalFmt e vm2Fmt não tenham R$\u00a0 (formato antigo)
  var fixFmt = function(s){ if(!s)return'—'; return s.replace('R$\u00a0','R$ ').replace('R$\u00A0','R$ '); };
  ...
  // ══════════════════════════════════════════════════════
  // S8 — ENCERRAMENTO (azul, sem dark dominante)
  // ══════════════════════════════════════════════════════
  var precoNovo=temPrec&&prec[prec.recomendacao]?fixFmt(prec[prec.recomendacao].totalFmt):(temPrec?fixFmt(prec.mercado.totalFmt):null);
  ...
  slides.push(slide('s8r',
    ...
    +'<div style="background:'+BLUE+';padding:56px 64px;display:flex;flex-direction:column;justify-content:center;gap:14px;position:relative;overflow:hidden">'
      ...
      +(precoNovo
        ?'<div style="background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);padding:20px 22px;margin-bottom:4px">'
          +'<div style="font-size:.58rem;color:rgba(255,255,255,.5);letter-spacing:.1em;text-transform:uppercase;margin-bottom:8px">Faixa recomendada</div>'
          +'<div style="font-size:2rem;font-weight:700;color:'+GOLD+';letter-spacing:-.03em">'+e(precoNovo)+'</div>'
          +(ajustePct!==null?'<div style="font-size:.62rem;color:rgba(255,255,255,.4);margin-top:5px">Ajuste de '+ajustePct+'% sobre o preço atual</div>':'')
        +'</div>'
        :'')
      +[
        ...
        return '<div style="background:rgba(255,255,255,.08);padding:14px 16px;display:flex;gap:12px;align-items:flex-start">'
          ...
          +'<div>'
            +'<div style="font-size:.8rem;font-weight:700;color:'+W+';margin-bottom:3px">'+item.title+'</div>'
            +'<div style="font-size:.7rem;color:rgba(255,255,255,.6);line-height:1.55">'+item.desc+'</div>'
          +'</div>'
        +'</div>';
      }).join('')
    +'</div>'
