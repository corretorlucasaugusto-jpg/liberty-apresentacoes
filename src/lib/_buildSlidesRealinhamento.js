// _buildSlidesRealinhamento.js
export function buildSlidesRealinhamento(d, slides = []) {
  function e(s) {
    if (!s) return '';
    return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
  }

  if (!d) d = {};
  if (!d.residencial) d.residencial = 'Residencial';
  if (!d.nome) d.nome = 'Proprietário';
  if (!d.nv) d.nv = [];
  if (!d.v) d.v = [];
  if (!d.visitas) d.visitas = [];
  if (!d.propostas) d.propostas = [];
  if (!d.acoes) d.acoes = [];

  // ── Paleta Liberty ──
  var C = {
    dark:   '#1d1d1f',
    blue:   '#1A4C8B',
    gold:   '#C9A84C',
    gray:   '#6e6e73',
    lgray:  '#a1a1a6',
    light:  '#f5f5f7',
    border: '#e2e2e7',
    white:  '#ffffff',
  };

  // logos inline base64
  var LOGO       = '<img src="data:image/webp;base64," style="width:64px;opacity:.9" alt="Liberty Imóveis"/>';
  var LOGO_DARK  = '<img src="data:image/webp;base64," style="width:56px;filter:brightness(0) invert(1);opacity:.7" alt="Liberty Imóveis"/>';
  var LOGO_SM    = '<img src="data:image/webp;base64," style="width:44px;opacity:.85" alt="Liberty Imóveis"/>';

  // ── Helpers ──
  var parseVal  = function(s){ return parseInt((s||'').replace(/[^0-9]/g,''))||0; };
  var parseArea = function(s){ return parseFloat((s||'').replace(/[^0-9.,]/g,'').replace(',','.'))||0; };

  var fmtData = function(iso) {
    if (!iso) return '—';
    try { var p=iso.split('-'); if(p.length===3) return p[2]+'/'+p[1]+'/'+p[0]; } catch(x){}
    return iso;
  };

  var fmtDias = function(n) {
    if (!n||n<=0) return '—';
    var m=Math.floor(n/30), r=n%30;
    if (m===0) return n+' dias';
    if (r===0) return m+(m>1?' meses':' mês');
    return m+(m>1?' meses':' mês')+' e '+r+' dias';
  };

  var fmtBRL = function(n) {
    if (!n||n<=0) return '—';
    return 'R$ '+Number(n).toLocaleString('pt-BR');
  };

  var diasEntre = function(a,b) {
    try { return Math.round(Math.abs(new Date(b)-new Date(a))/864e5); } catch(x){ return 0; }
  };

  var diasCampanha = d.data_inicio ? diasEntre(d.data_inicio, new Date().toISOString().split('T')[0]) : 0;
  var totalVisitantes = (d.visitas||[]).reduce(function(a,v){ return a+(parseInt(v.qtd)||0); },0);
  var propostasFilt   = (d.propostas||[]).filter(function(p){ return p.valor||p.data; });
  var totalPropostas  = propostasFilt.length;
  var maiorProposta   = propostasFilt.reduce(function(a,p){ var v=parseVal(p.valor); return v>a?v:a; },0);
  var precoOrigNum    = parseVal(d.preco_original);
  var selicNum        = parseFloat((d.selic||'14,50').replace(',','.'))||14.5;
  var custoMensal     = precoOrigNum>0 ? Math.round(precoOrigNum*(selicNum/100)/12) : 0;
  var custoTotal      = Math.round(custoMensal * Math.max(1, diasCampanha/30));
  var acoesFilt       = (d.acoes||[]).filter(Boolean);

  var nvVals  = (d.nv||[]).map(function(r){return parseVal(r.v);}).filter(Boolean);
  var nvMedio = nvVals.length>0 ? Math.round(nvVals.reduce(function(a,b){return a+b;},0)/nvVals.length) : 0;

  // ── Layout helpers ──
  var slide = function(id, inner, bg) {
    return '<div class="slide" id="'+id+'" style="display:flex;flex-direction:column;height:100%;overflow:hidden;background:'+(bg||C.white)+'">'+inner+'</div>';
  };

  var head = function(label, title, sub) {
    return '<div style="padding:36px 56px 22px;flex-shrink:0">'+
      '<div style="font-size:.5rem;font-weight:700;letter-spacing:.24em;text-transform:uppercase;color:'+C.gold+';margin-bottom:10px">'+label+'</div>'+
      '<div style="display:flex;justify-content:space-between;align-items:flex-end;gap:24px">'+
        '<div style="font-size:clamp(1.7rem,2.8vw,2.6rem);font-weight:800;color:'+C.dark+';line-height:.95;letter-spacing:-.04em">'+title+'</div>'+
        (sub?'<div style="font-size:.7rem;color:'+C.lgray+';text-align:right;line-height:1.5;max-width:200px;flex-shrink:0">'+sub+'</div>':'')+
      '</div>'+
      '<div style="height:1px;background:'+C.border+';margin-top:18px"></div>'+
    '</div>';
  };

  var body = function(inner) {
    return '<div style="flex:1;overflow-y:auto;overflow-x:hidden;padding:0 56px 36px">'+inner+'</div>';
  };

  var diasColor = function(dd) {
    var n=parseInt((dd||'').replace(/[^0-9]/g,''))||0;
    if (!n) return C.lgray;
    if (n<30)  return '#27ae60';
    if (n<90)  return '#b45309';
    if (n<180) return '#c0392b';
    return '#7f0000';
  };


  // ══════════════════════════════════════════════════════
  // S1 — CAPA
  // ══════════════════════════════════════════════════════
  var kpis = [
    ['Início da campanha', fmtData(d.data_inicio)],
    ['Tempo em campanha',  diasCampanha>0 ? fmtDias(diasCampanha) : '—'],
    ['Visitantes',         totalVisitantes>0 ? totalVisitantes : '—'],
    ['Propostas recebidas',totalPropostas>0 ? totalPropostas : '0'],
  ];

  slides.push(slide('s1r',
    '<div style="display:grid;grid-template-columns:1fr 1fr;height:100%">' +

    // Esquerda escura
    '<div style="background:'+C.dark+';padding:64px 60px;display:flex;flex-direction:column;justify-content:space-between">' +
      '<div style="display:flex;align-items:center;gap:12px">' +
        LOGO_DARK +
        '<div style="width:1px;height:32px;background:rgba(255,255,255,.15)"></div>' +
        '<span style="font-size:.54rem;font-weight:600;letter-spacing:.2em;text-transform:uppercase;color:rgba(255,255,255,.3)">Brasília · DF</span>' +
      '</div>' +
      '<div>' +
        '<div style="font-size:.52rem;font-weight:700;letter-spacing:.22em;text-transform:uppercase;color:'+C.gold+';margin-bottom:18px">Alinhamento Estratégico</div>' +
        '<div style="font-size:clamp(3rem,5.5vw,6rem);font-weight:900;color:'+C.white+';line-height:.82;letter-spacing:-.06em;margin-bottom:28px">' +
          'Análise<br>de<br><span style="color:'+C.gold+'">Campanha</span>' +
        '</div>' +
        '<div style="font-size:.86rem;color:rgba(255,255,255,.38);line-height:1.65">'+e(d.residencial)+(d.bairro?' &middot; '+e(d.bairro):'')+'</div>' +
      '</div>' +
      '<div style="font-size:.54rem;color:rgba(255,255,255,.18);letter-spacing:.06em">Preparado para '+e(d.nome)+' &middot; '+new Date().toLocaleDateString('pt-BR')+'</div>' +
    '</div>' +

    // Direita clara
    '<div style="background:'+C.light+';padding:64px 56px;display:flex;flex-direction:column;justify-content:center">' +
      '<div style="font-size:.5rem;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:'+C.lgray+';margin-bottom:24px">Resumo</div>' +
      '<div style="display:flex;flex-direction:column;gap:1px;background:'+C.border+'">' +
        kpis.map(function(k){
          return '<div style="background:'+C.white+';padding:20px 24px;display:flex;justify-content:space-between;align-items:center">' +
            '<div style="font-size:.76rem;color:'+C.gray+'">'+k[0]+'</div>' +
            '<div style="font-size:1.05rem;font-weight:700;color:'+C.dark+'">'+e(String(k[1]))+'</div>' +
          '</div>';
        }).join('') +
        (precoOrigNum>0 ?
          '<div style="background:'+C.dark+';padding:20px 24px;display:flex;justify-content:space-between;align-items:center">' +
            '<div style="font-size:.76rem;color:rgba(255,255,255,.38)">Preço anunciado</div>' +
            '<div style="font-size:1.05rem;font-weight:700;color:'+C.gold+'">'+fmtBRL(precoOrigNum)+'</div>' +
          '</div>'
        :'') +
        (maiorProposta>0 ?
          '<div style="background:#fefce8;padding:20px 24px;display:flex;justify-content:space-between;align-items:center;border-top:2px solid '+C.gold+'">' +
            '<div style="font-size:.76rem;color:'+C.gray+'">Melhor proposta recebida</div>' +
            '<div style="font-size:1.05rem;font-weight:700;color:#92400e">'+fmtBRL(maiorProposta)+'</div>' +
          '</div>'
        :'') +
      '</div>' +
    '</div>' +

    '</div>'
  ));


  // ══════════════════════════════════════════════════════
  // S2 — LINHA DO TEMPO
  // ══════════════════════════════════════════════════════
  var events = [];

  if (d.data_inicio) events.push({
    iso: d.data_inicio, icon: '🚀',
    title: 'Início da campanha',
    sub: precoOrigNum>0 ? 'Anunciado por '+fmtBRL(precoOrigNum) : '',
    cor: C.blue,
  });

  acoesFilt.forEach(function(a){
    events.push({ iso:'', icon:'✓', title:a, sub:'', cor:C.gold });
  });

  (d.visitas||[]).forEach(function(v){
    if (v.data||v.qtd||v.feedback) events.push({
      iso: v.data||'',
      icon: '👥',
      title: (v.qtd?v.qtd+' visitante'+(parseInt(v.qtd)>1?'s':''):'Visita'),
      sub: v.feedback ? e(v.feedback.slice(0,110))+(v.feedback.length>110?'…':'') : '',
      cor: C.blue,
    });
  });

  propostasFilt.forEach(function(p){
    events.push({
      iso: p.data||'',
      icon: '📋',
      title: 'Proposta recebida'+(p.valor?' &middot; '+e(p.valor):''),
      sub:'', cor: C.gold,
    });
  });

  events.sort(function(a,b){
    if (!a.iso&&!b.iso) return 0;
    if (!a.iso) return 1; if (!b.iso) return -1;
    return new Date(a.iso)-new Date(b.iso);
  });

  var col1=[],col2=[];
  events.forEach(function(ev,i){ (i%2===0?col1:col2).push(ev); });

  var renderEv = function(ev, isLast) {
    var evBorder = isLast ? '' : 'border-bottom:1px solid '+C.border+';';
    return '<div style="display:flex;gap:14px;padding:16px 0;"+evBorder+"">' +
      '<div style="width:34px;height:34px;border-radius:50%;background:'+ev.cor+'15;border:1.5px solid '+ev.cor+'35;display:flex;align-items:center;justify-content:center;font-size:.88rem;flex-shrink:0">'+ev.icon+'</div>' +
      '<div style="flex:1;min-width:0">' +
        (ev.iso?'<div style="font-size:.56rem;font-weight:600;letter-spacing:.07em;text-transform:uppercase;color:'+C.lgray+';margin-bottom:3px">'+fmtData(ev.iso)+'</div>':'') +
        '<div style="font-size:.8rem;font-weight:700;color:'+C.dark+';margin-bottom:'+(ev.sub?'4px':'0')+'">'+ev.title+'</div>' +
        (ev.sub?'<div style="font-size:.68rem;color:'+C.gray+';line-height:1.5;font-style:italic">&ldquo;'+ev.sub+'&rdquo;</div>':'') +
      '</div>' +
    '</div>';
  };

  slides.push(slide('s2r',
    head('Histórico', 'Linha do tempo da campanha', 'Tudo o que aconteceu desde o lançamento.') +
    body(
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0 40px">' +
        '<div>' + col1.map(function(ev,i){return renderEv(ev,i===col1.length-1);}).join('') + '</div>' +
        '<div style="border-left:1px solid '+C.border+';padding-left:40px">' +
          col2.map(function(ev,i){return renderEv(ev,i===col2.length-1);}).join('') +
        '</div>' +
      '</div>'
    )
  ));


  // ══════════════════════════════════════════════════════
  // S3 — DIAGNÓSTICO (une dados + ações em um só slide)
  // ══════════════════════════════════════════════════════
  var posMsg = '';
  if (precoOrigNum>0 && nvMedio>0) {
    var diff=precoOrigNum-nvMedio, pct=Math.abs(Math.round((diff/nvMedio)*100));
    posMsg = diff>0
      ? 'O imóvel está anunciado por <strong>'+fmtBRL(Math.abs(diff))+' ('+pct+'%) acima</strong> da média dos concorrentes — o comprador vê alternativas mais acessíveis primeiro.'
      : 'O preço está dentro da faixa de mercado. Outros fatores podem estar influenciando a decisão.';
  }

  slides.push(slide('s3r',
    head('Diagnóstico', 'O que os dados indicam', 'Fatos objetivos da campanha.') +
    body(
      // Big numbers — linha única
      '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px">' +
        [
          { n: diasCampanha||'—', unit:'dias em campanha', cor: diasCampanha>=90?'#c0392b':diasCampanha>=45?'#b45309':C.blue, sub: diasCampanha<30?'Dentro do prazo':diasCampanha<90?'Acima do prazo médio':'Risco de desvalorização de percepção' },
          { n: totalVisitantes||'—', unit:'visitante'+(totalVisitantes!==1?'s':''), cor: C.blue, sub: totalVisitantes>0?'Interesse comprovado':'Sem visitas registradas' },
          { n: totalPropostas||'0', unit:'proposta'+(totalPropostas!==1?'s':''), cor: C.gold, sub: maiorProposta>0&&precoOrigNum>0?'Melhor: '+fmtBRL(maiorProposta)+' ('+Math.round((maiorProposta/precoOrigNum)*100)+'% do pedido)':'Abaixo do valor pedido' },
        ].map(function(s){
          return '<div style="background:'+C.light+';border-radius:14px;padding:24px 22px;border-top:3px solid '+s.cor+'">' +
            '<div style="font-size:clamp(2rem,3.5vw,3.2rem);font-weight:900;color:'+s.cor+';line-height:1;letter-spacing:-.05em;margin-bottom:6px">'+s.n+'</div>' +
            '<div style="font-size:.74rem;font-weight:700;color:'+C.dark+';margin-bottom:6px">'+s.unit+'</div>' +
            '<div style="font-size:.65rem;color:'+C.gray+';line-height:1.5">'+s.sub+'</div>' +
          '</div>';
        }).join('') +
      '</div>' +

      // Duas colunas: ações | sinais
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">' +

        // Ações
        '<div style="background:'+C.light+';border-radius:14px;padding:20px 22px">' +
          '<div style="font-size:.52rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:'+C.blue+';margin-bottom:14px">Ações realizadas ('+acoesFilt.length+')</div>' +
          acoesFilt.map(function(a,i){
            var acaoBorder = i>0 ? 'border-top:1px solid '+C.border+';' : '';
            return '<div style="display:flex;align-items:center;gap:10px;padding:9px 0;'+acaoBorder+'">' +
              '<div style="width:18px;height:18px;border-radius:5px;background:'+C.blue+'20;display:flex;align-items:center;justify-content:center;flex-shrink:0">' +
                '<svg width="9" height="9" viewBox="0 0 9 9" fill="none" stroke="'+C.blue+'" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="1,4.5 3.2,7 8,2"/></svg>' +
              '</div>' +
              '<div style="font-size:.76rem;color:'+C.dark+'">'+e(a)+'</div>' +
            '</div>';
          }).join('') +
        '</div>' +

        // Sinais e feedbacks
        '<div style="display:flex;flex-direction:column;gap:10px">' +
          (posMsg?
            '<div style="background:#fffbeb;border-left:3px solid '+C.gold+';border-radius:0 10px 10px 0;padding:14px 16px">' +
              '<div style="font-size:.52rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:'+C.gold+';margin-bottom:6px">Posicionamento</div>' +
              '<div style="font-size:.74rem;color:'+C.dark+';line-height:1.65">'+posMsg+'</div>' +
            '</div>'
          :'') +
          // feedbacks das visitas
          (d.visitas||[]).filter(function(v){return v.feedback;}).map(function(v){
            return '<div style="background:'+C.light+';border-radius:10px;padding:14px 16px">' +
              (v.data?'<div style="font-size:.56rem;color:'+C.lgray+';margin-bottom:4px">'+fmtData(v.data)+'</div>':'') +
              '<div style="font-size:.74rem;color:'+C.dark+';line-height:1.6;font-style:italic">&ldquo;'+e(v.feedback)+'&rdquo;</div>' +
            '</div>';
          }).join('') +
        '</div>' +

      '</div>'
    )
  ));


  // ══════════════════════════════════════════════════════
  // S4 — COMPARATIVO DE MERCADO
  // ══════════════════════════════════════════════════════
  (function(){
    var rowHtml=function(r,i,isNV){
      var val=parseVal(r.v),area=parseArea(r.a);
      var vm2=area>0&&val>0?'R$ '+Math.round(val/area).toLocaleString('pt-BR')+'/m²':'—';
      var carac=r.c||[r.quartos?r.quartos+' qts':'',r.conservacao||''].filter(Boolean).join(' · ')||'—';
      var db=r.d?'<span style="font-size:.58rem;font-weight:700;padding:2px 7px;border-radius:20px;background:'+diasColor(r.d)+'18;color:'+diasColor(r.d)+'">'+e(r.d)+'</span>':'—';
      var btn=isNV&&r.url?'<button onclick="openAnuncio(this.dataset.url)" data-url="'+(r.url||'').replace(/"/g,'&quot;')+'" style="font-size:.56rem;font-weight:700;padding:3px 8px;background:'+C.blue+';color:#fff;border:none;border-radius:5px;cursor:pointer">Ver</button>':'';
      var grid=isNV?'1.8fr .42fr 1.6fr .85fr .7fr .7fr .3fr':'1.8fr .42fr 1.6fr .85fr .7fr .7fr';
      var rowBorder = i>0 ? 'border-top:1px solid '+C.border+';' : '';
      return '<div style="display:grid;grid-template-columns:'+grid+';align-items:center;padding:9px 14px;background:'+(i%2===0?C.white:C.light)+';'+rowBorder+'">' +
        '<div style="font-size:.72rem;font-weight:600;color:'+C.dark+'">'+e(r.n||'—')+'</div>' +
        '<div style="font-size:.66rem;color:'+C.gray+'">'+e(r.a||'—')+'m²</div>' +
        '<div style="font-size:.62rem;color:'+C.gray+'">'+e(carac)+'</div>' +
        '<div style="font-size:.72rem;font-weight:700;color:'+(isNV?'#c0392b':'#27ae60')+'">'+e(r.v||'—')+'</div>' +
        '<div style="font-size:.6rem;color:'+C.lgray+'">'+vm2+'</div>' +
        '<div>'+db+'</div>' +
        (isNV?'<div>'+btn+'</div>':'') +
      '</div>';
    };

    var hdr=function(isNV){
      var grid=isNV?'1.8fr .42fr 1.6fr .85fr .7fr .7fr .3fr':'1.8fr .42fr 1.6fr .85fr .7fr .7fr';
      return '<div style="display:grid;grid-template-columns:'+grid+';padding:7px 14px;background:'+C.dark+';font-size:.54rem;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:rgba(255,255,255,.4)">' +
        '<div>Imóvel</div><div>Área</div><div>Características</div>' +
        (isNV?'<div>Anunciado</div>':'<div>Negociado</div>') +
        '<div>R$/m²</div><div>Dias</div>' + (isNV?'<div></div>':'') +
      '</div>';
    };

    var sec=function(cor,txt){
      return '<div style="font-size:.52rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:'+cor+';padding:14px 14px 6px;display:flex;align-items:center;gap:7px">' +
        '<span style="width:6px;height:6px;border-radius:50%;background:'+cor+'"></span>'+txt+
      '</div>';
    };

    var nvLocal=(d.nv||[]).filter(function(r){return (r.cat||'local')==='local';});
    var nvAmplo=(d.nv||[]).filter(function(r){return r.cat==='amplo';});
    var localRows=nvLocal.map(function(r,i){return rowHtml(r,i,true);}).join('');
    var amploRows=nvAmplo.map(function(r,i){return rowHtml(r,i,true);}).join('');
    var vRows=(d.v||[]).map(function(r,i){return rowHtml(r,i,false);}).join('');
    if(!localRows&&!amploRows) localRows='<div style="padding:14px;font-size:.74rem;color:'+C.lgray+'">Nenhum comparável cadastrado.</div>';

    slides.push(slide('s4r',
      head('Mercado', 'O que o comprador está vendo', 'Concorrentes ativos e vendas recentes.') +
      '<div style="flex:1;overflow-y:auto;padding:0 56px 36px">' +
        (vRows?'<div style="margin-bottom:16px;border:1px solid '+C.border+';border-radius:12px;overflow:hidden">'+sec('#27ae60','Vendidos recentes')+hdr(false)+vRows+'</div>':'')+
        '<div style="margin-bottom:16px;border:1px solid '+C.border+';border-radius:12px;overflow:hidden">'+sec(C.blue,'Concorrentes ativos')+hdr(true)+localRows+'</div>'+
        (amploRows?'<div style="border:1px solid '+C.border+';border-radius:12px;overflow:hidden">'+sec(C.gold,'Também no radar')+hdr(true)+amploRows+'</div>':'')+
      '</div>'
    ));
  })();


  // ══════════════════════════════════════════════════════
  // S5 — CUSTO DO TEMPO
  // ══════════════════════════════════════════════════════
  slides.push(slide('s5r',
    '<div style="display:grid;grid-template-columns:1fr 1fr;height:100%">' +

    // Esquerda escura — número de impacto
    '<div style="background:'+C.dark+';padding:64px 60px;display:flex;flex-direction:column;justify-content:center">' +
      '<div style="font-size:.52rem;font-weight:700;letter-spacing:.22em;text-transform:uppercase;color:'+C.gold+';margin-bottom:22px">Custo acumulado</div>' +
      '<div style="font-size:clamp(2.6rem,5vw,5.2rem);font-weight:900;color:'+C.white+';line-height:.88;letter-spacing:-.05em;margin-bottom:14px">'+fmtBRL(custoTotal)+'</div>' +
      '<div style="font-size:.78rem;color:rgba(255,255,255,.38);line-height:1.7;margin-bottom:36px">'+fmtDias(diasCampanha)+' × Selic <strong style="color:rgba(255,255,255,.6)">'+e(d.selic||'14,50%')+'</strong>/ano sobre o valor anunciado.'+'</div>' +
      '<div style="height:1px;background:rgba(255,255,255,.1);margin-bottom:28px"></div>' +
      '<div style="display:flex;gap:28px">' +
        '<div><div style="font-size:1.5rem;font-weight:800;color:'+C.gold+'">'+fmtBRL(custoMensal)+'</div><div style="font-size:.58rem;color:rgba(255,255,255,.28);margin-top:3px">por mês</div></div>' +
        '<div><div style="font-size:1.5rem;font-weight:800;color:rgba(255,255,255,.6)">'+diasCampanha+'</div><div style="font-size:.58rem;color:rgba(255,255,255,.28);margin-top:3px">dias</div></div>' +
      '</div>' +
    '</div>' +

    // Direita — argumentos
    '<div style="background:'+C.light+';padding:64px 56px;display:flex;flex-direction:column;justify-content:center;gap:12px">' +
      '<div style="font-size:.52rem;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:'+C.gold+';margin-bottom:8px">O que isso representa</div>' +
      [
        {cor:C.blue,  title:'Dinheiro que deixa de render', desc:'Cada mês sem venda representa '+fmtBRL(custoMensal)+' em custo de oportunidade. Esse valor cresce enquanto o imóvel aguarda.'},
        {cor:C.gold,  title:'O comprador percebe a demora', desc:'Após 90 dias anunciado, o imóvel começa a gerar dúvida no comprador antes mesmo da visita.'},
        {cor:C.dark,  title:maiorProposta>0&&precoOrigNum>0?'A proposta está perto':'O interesse existe',
          desc: maiorProposta>0&&precoOrigNum>0
            ?'A diferença entre o preço pedido e a melhor proposta é de '+fmtBRL(precoOrigNum-maiorProposta)+'. O custo de esperar pode superar essa diferença.'
            :'As visitas aconteceram. Há demanda real — o preço é o único ponto de ajuste.'
        },
      ].map(function(item){
        return '<div style="background:'+C.white+';border-radius:12px;padding:18px 20px;border-left:3px solid '+item.cor+'">' +
          '<div style="font-size:.78rem;font-weight:700;color:'+C.dark+';margin-bottom:5px">'+item.title+'</div>' +
          '<div style="font-size:.7rem;color:'+C.gray+';line-height:1.6">'+item.desc+'</div>' +
        '</div>';
      }).join('') +
    '</div>' +

    '</div>'
  ));


  // ══════════════════════════════════════════════════════
  // S6 — FAIXA DE PREÇO
  // ══════════════════════════════════════════════════════
  var prec=d.prec||null;
  var temPrec=prec&&prec.mercado&&prec.mercado.total;

  slides.push(slide('s6r',
    head('Precificação', 'Faixa de referência do mercado', 'Baseada nos dados reais de comparáveis.') +
    body(
      temPrec?(
        '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:20px">' +
          [
            {key:'competitivo',label:'Competitivo',sub:'Fechar em até 30 dias',cor:'#27ae60'},
            {key:'mercado',    label:'Mercado',    sub:'Equilíbrio · 30–60 dias',cor:C.blue},
            {key:'otimista',   label:'Otimista',   sub:'Com margem · 60–90 dias',cor:C.gold},
          ].map(function(f){
            var faixa=prec[f.key]; if(!faixa) return '';
            var isRec=prec.recomendacao===f.key;
            var ajuste='';
            if(precoOrigNum>0&&faixa.total){
              var dif=faixa.total-precoOrigNum, pct=Math.abs(Math.round((dif/precoOrigNum)*100));
              ajuste='<div style="margin-top:12px;padding-top:12px;border-top:1px solid '+C.border+';font-size:.63rem;color:'+f.cor+';font-weight:700">' +
                (dif<0?'↓ Ajuste de '+pct+'% abaixo do atual':'↑ Acima do atual em '+pct+'%')+
              '</div>';
            }
            // Badge DENTRO do card, não absoluto
            return '<div style="background:'+C.light+';border-radius:14px;padding:22px;border-top:3px solid '+f.cor+(isRec?';box-shadow:0 4px 20px rgba(0,0,0,.08)':'')+'">' +
              (isRec?'<div style="display:inline-block;background:'+f.cor+';color:#fff;font-size:.48rem;font-weight:700;padding:3px 10px;border-radius:20px;letter-spacing:.12em;text-transform:uppercase;margin-bottom:12px">✦ Recomendado</div><br>':'') +
              '<div style="font-size:.52rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:'+f.cor+';margin-bottom:10px">'+f.label+'</div>' +
              '<div style="font-size:1.8rem;font-weight:900;color:'+C.dark+';line-height:1;letter-spacing:-.04em;margin-bottom:5px">'+e(faixa.totalFmt||'—')+'</div>' +
              '<div style="font-size:.64rem;color:'+C.lgray+';margin-bottom:8px">'+e(faixa.vm2Fmt||'—')+'</div>' +
              '<div style="font-size:.68rem;color:'+C.gray+';line-height:1.5">'+f.sub+'</div>' +
              ajuste +
            '</div>';
          }).join('') +
        '</div>' +
        (prec.justificativa?
          '<div style="background:'+C.light+';border-radius:12px;padding:18px 22px">' +
            '<div style="font-size:.52rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:'+C.blue+';margin-bottom:7px">Análise</div>' +
            '<div style="font-size:.76rem;color:'+C.dark+';line-height:1.7">'+e(prec.justificativa)+'</div>' +
          '</div>'
        :'')
      ):(
        '<div style="background:'+C.light+';border-radius:16px;padding:48px;text-align:center">' +
          '<div style="font-size:2.2rem;margin-bottom:14px">📊</div>' +
          '<div style="font-size:.88rem;font-weight:700;color:'+C.dark+';margin-bottom:8px">Precificação não calculada</div>' +
          '<div style="font-size:.74rem;color:'+C.gray+';line-height:1.6;max-width:280px;margin:0 auto">Preencha os concorrentes e clique em Precificar no formulário.</div>' +
        '</div>'
      )
    )
  ));


  // ══════════════════════════════════════════════════════
  // S7 — ENCERRAMENTO
  // ══════════════════════════════════════════════════════
  var precoNovo = temPrec&&prec[prec.recomendacao] ? prec[prec.recomendacao].totalFmt : (temPrec?prec.mercado.totalFmt:null);
  var ajustePct = temPrec&&prec[prec.recomendacao]&&precoOrigNum>0
    ? Math.abs(Math.round(((prec[prec.recomendacao].total-precoOrigNum)/precoOrigNum)*100)) : null;

  slides.push(slide('s7r',
    '<div style="display:grid;grid-template-columns:1fr 1fr;height:100%">' +

    // Esquerda escura
    '<div style="background:'+C.dark+';padding:64px 60px;display:flex;flex-direction:column;justify-content:space-between">' +
      '<div>' + LOGO_DARK + '</div>' +
      '<div>' +
        '<div style="font-size:.52rem;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:'+C.gold+';margin-bottom:18px">Conclusão</div>' +
        '<div style="font-size:clamp(2.8rem,4.5vw,5rem);font-weight:900;color:'+C.white+';line-height:.85;letter-spacing:-.06em;margin-bottom:28px">' +
          'A decisão<br>é sua.<br><span style="color:'+C.gold+'">Os dados<br>já falaram.</span>' +
        '</div>' +
        '<div style="font-size:.82rem;color:rgba(255,255,255,.38);line-height:1.75;max-width:300px">' +
          'Não é uma pressão — é o mercado comunicando, de forma objetiva, qual o caminho para o fechamento.' +
        '</div>' +
      '</div>' +
      '<div>' +
        (precoNovo?
          '<div style="background:rgba(255,255,255,.06);border-radius:10px;padding:20px 22px;margin-bottom:16px">' +
            '<div style="font-size:.5rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:rgba(255,255,255,.28);margin-bottom:7px">Faixa recomendada</div>' +
            '<div style="font-size:2.2rem;font-weight:900;color:'+C.gold+';letter-spacing:-.04em">'+e(precoNovo)+'</div>' +
            (ajustePct!==null?'<div style="font-size:.62rem;color:rgba(255,255,255,.25);margin-top:5px">Ajuste de '+ajustePct+'% sobre o preço atual</div>':'') +
          '</div>'
        :'') +
        '<div style="font-size:.5rem;color:rgba(255,255,255,.14);letter-spacing:.08em">Liberty Imóveis · Brasília, DF</div>' +
      '</div>' +
    '</div>' +

    // Direita — argumentos
    '<div style="background:'+C.light+';padding:64px 52px;display:flex;flex-direction:column;justify-content:center;gap:12px">' +
      '<div style="font-size:.52rem;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:'+C.gold+';margin-bottom:10px">Por que agir agora</div>' +
      [
        {icon:'⏱', title:'O tempo tem custo', desc:fmtBRL(custoMensal)+' por mês em custo de oportunidade. Cresce a cada semana.'},
        {icon:'👥', title:'O interesse está comprovado', desc:(totalVisitantes>0?totalVisitantes+' pessoa'+(totalVisitantes>1?'s':'')+' visitaram o imóvel.':'Visitantes chegaram.')+' O preço bloqueou a decisão de compra.'},
        {icon:'📉', title:'O mercado penaliza a demora', desc:'Acima de 90 dias, o imóvel passa a gerar dúvida antes mesmo da visita.'},
        {icon:'✓',  title:'A venda está ao alcance', desc:'Localização, produto e campanha estão prontos. Um único ajuste fecha esse ciclo.'},
      ].map(function(item){
        return '<div style="display:flex;gap:14px;background:'+C.white+';border-radius:12px;padding:16px 18px">' +
          '<div style="width:34px;height:34px;border-radius:9px;background:'+C.gold+'18;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:1rem">'+item.icon+'</div>' +
          '<div>' +
            '<div style="font-size:.78rem;font-weight:700;color:'+C.dark+';margin-bottom:4px">'+item.title+'</div>' +
            '<div style="font-size:.68rem;color:'+C.gray+';line-height:1.55">'+item.desc+'</div>' +
          '</div>' +
        '</div>';
      }).join('') +
    '</div>' +

    '</div>'
  ));

  return slides;
}
