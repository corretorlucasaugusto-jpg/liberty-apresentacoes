...
export function buildSlidesRealinhamento(d, slides = []) {
  ...
  slides.push(slide('s1r',
    ...
    +'<div style="padding:56px 64px;display:flex;flex-direction:column;justify-content:space-between;border-right:1px solid '+BL+'">'
      ...
      +'<div>'
        +'<div style="font-size:.6rem;color:'+LGRAY+';letter-spacing:.16em;text-transform:uppercase;font-weight:500;margin-bottom:14px">Liberty Imóveis</div>'
    ...
    +'<div style="background:'+DARK+';padding:56px 64px;display:flex;flex-direction:column;justify-content:space-between;position:relative;overflow:hidden">'
      ...
      +'<div style="font-size:.58rem;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:rgba(255,255,255,.28)">Resumo</div>'
      +'<div style="display:flex;flex-direction:column;gap:1px;background:rgba(255,255,255,.08);position:relative;z-index:1">'
        ...
        +(precoOrigNum>0?'<div style="background:rgba(184,131,42,.15);padding:18px 22px;display:flex;justify-content:space-between;align-items:center;border-top:1px solid rgba(184,131,42,.3)"><div style="font-size:.76rem;color:rgba(255,255,255,.45)">Preço anunciado</div><div style="font-size:1rem;font-weight:700;color:'+GOLD+'">'+fmtBRL(precoOrigNum)+'</div></div>':'')
        +(maiorProposta>0?'<div style="background:rgba(192,57,43,.12);padding:18px 22px;display:flex;justify-content:space-between;align-items:center"><div style="font-size:.76rem;color:rgba(255,255,255,.45)">Melhor proposta</div><div style="font-size:1rem;font-weight:700;color:#e88">'+fmtBRL(maiorProposta)+'</div></div>':'')
      +'</div>'
  ...
  // ══════════════════════════════════════════════════════
  // S2 — LINHA DO TEMPO (só visitas, início, propostas)
  // ══════════════════════════════════════════════════════
  var events = [];
  ...
  (d.visitas||[]).forEach(function(v){
    if(v.data||v.qtd||v.feedback) events.push({
      ...
      sub:v.feedback?e(v.feedback.slice(0,70))+(v.feedback.length>70?'…':''):'',
      cor:BLUE
    });
  });
  ...
  var evHtml = events.map(function(ev,i){
    ...
    return '<div style="display:flex;flex-direction:column;align-items:center;flex:1;min-width:130px;max-width:180px;position:relative">'
      ...
      +'<div style="width:40px;height:40px;border-radius:50%;background:'+W+';border:2px solid '+ev.cor+';display:flex;align-items:center;justify-content:center;font-size:.9rem;position:relative;z-index:1">'+ev.icon+'</div>'
      ...
      +'<div style="font-size:.76rem;font-weight:700;color:'+DARK+';margin-top:4px;text-align:center;line-height:1.3;padding:0 6px">'+ev.title+'</div>'
      +(ev.sub?'<div style="font-size:.62rem;color:'+GRAY+';margin-top:4px;text-align:center;line-height:1.4;font-style:italic;padding:0 6px">"'+ev.sub+'"</div>':'')
    +'</div>';
  }).join('');

  slides.push(slide('s2r',
    sh('Histórico', GOLD, 'Linha do tempo', 'Visitas e propostas desde o lançamento.')
    +'<div style="flex:1;display:flex;align-items:center;padding:0 64px">'
      +'<div style="overflow-x:auto;width:100%;padding:16px 0">'
        +'<div style="display:flex;align-items:flex-start;min-width:'+(events.length*160)+'px;gap:0">'
          +evHtml
        +'</div>'
      +'</div>'
    +'</div>'
  ...
  // ══════════════════════════════════════════════════════
  // S3 — AÇÕES REALIZADAS (com descrições IA)
  // ══════════════════════════════════════════════════════
  // Ícones por categoria (mapeamento por palavra-chave)
  var getIcon = function(acao) {
    var a = (acao||'').toLowerCase();
    if (/foto|imagem|photo/.test(a))      return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="12" cy="12" r="3"/><path d="M8.5 5l1.5-2h4l1.5 2"/></svg>';
    if (/v[íi]deo|video|film/.test(a))    return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="15" height="12" rx="2"/><path d="M17 9l5-3v12l-5-3V9z"/></svg>';
    if (/portal|zap|viva|imovel|site|anun/.test(a)) return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M2 12h20M12 2a15 15 0 010 20M12 2a15 15 0 000 20"/></svg>';
    if (/tr[áa]fego|anún|social|instagram|facebook|mídia|midia/.test(a)) return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M22 4s-2.5 2-4 2c-1.7-2-4.3-2-6 0S9.7 10 8 10c-1.5 0-4-2-4-2"/><path d="M2 20s2.5-2 4-2c1.7 2 4.3 2 6 0s2.3-4 4-4c1.5 0 4 2 4 2"/></svg>';
    if (/placa|fachada|banner|outdoor/.test(a)) return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="12" rx="1"/><path d="M8 20h8M12 16v4"/></svg>';
    if (/vizinho|prospec|abordagem|contact/.test(a)) return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>';
    if (/whatsapp|mensagem|disparo/.test(a)) return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>';
    if (/drone|a[eé]reo/.test(a))          return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 12m-2 0a2 2 0 104 0 2 2 0 10-4 0"/><path d="M4 8V6a2 2 0 012-2h2M4 16v2a2 2 0 002 2h2M20 8V6a2 2 0 00-2-2h-2M20 16v2a2 2 0 01-2 2h-2"/></svg>';
    // default
    return '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>';
  };

  var acaoCards = acoesFilt.map(function(acao, i) {
    var desc = (d.acoesDesc && d.acoesDesc[acao]) || '';
    return '<div style="background:'+OFF+';padding:22px 20px;display:flex;gap:16px;align-items:flex-start">'
      +'<div style="width:48px;height:48px;background:'+BLUE+'12;border:1.5px solid '+BLUE+'30;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:'+BLUE+'">'
        +getIcon(acao)
      +'</div>'
      +'<div style="flex:1;min-width:0">'
        +'<div style="font-size:.82rem;font-weight:700;color:'+DARK+';margin-bottom:'+(desc?'5px':'0')+'">'+e(acao)+'</div>'
        +(desc?'<div style="font-size:.72rem;color:'+GRAY+';line-height:1.6">'+e(desc)+'</div>':'')
      +'</div>'
    +'</div>';
  }).join('');

  var nCols = acoesFilt.length <= 3 ? 1 : 2;
  var gridStyle = nCols === 2
    ? 'display:grid;grid-template-columns:1fr 1fr;gap:1px;background:'+BORD
    : 'display:flex;flex-direction:column;gap:1px;background:'+BORD;

  slides.push(slide('s3r',
    sh('Campanha', BLUE, 'O que foi feito por você', 'Cada ação, um passo rumo ao fechamento.')
    +sb(
      '<div style="'+gridStyle+'">'+acaoCards+'</div>'
      +(acoesFilt.length===0?'<div style="padding:32px;font-size:.8rem;color:'+LGRAY+';text-align:center">Nenhuma ação registrada.</div>':'')
    )
  ));
  ...
  (function(){
    var rowHtml=function(r,i,isNV){
      ...
      var btn=isNV&&r.url?'<button onclick="openAnuncio(this.dataset.url)" data-url="'+(r.url||'').replace(/"/g,'&quot;')+'" style="font-size:.58rem;font-weight:600;padding:3px 8px;background:'+BLUE+';color:'+W+';border:none;cursor:pointer">Ver</button>':'';
      var grid=isNV?'1.8fr .42fr 1.6fr .85fr .7fr .7fr .3fr':'1.8fr .42fr 1.6fr .85fr .7fr .7fr';
      var rowBorder=i>0?'border-top:1px solid '+BL+';':'';
      return '<div style="display:grid;grid-template-columns:'+grid+';align-items:center;padding:9px 14px;background:'+(i%2===0?W:OFF)+';'+rowBorder+'">'
    ...
    var hdr=function(isNV){
      ...
      return '<div style="display:grid;grid-template-columns:'+grid+';padding:7px 14px;background:'+BLUE+';font-size:.58rem;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:rgba(255,255,255,.85)">'
  ...
  slides.push(slide('s7r',
    ...
    +sb(
      temPrec?(
        ...
        +'</div>'
        +(prec.justificativa?'<div style="background:'+GBG+';border-left:3px solid '+GOLD+';padding:16px 20px"><div style="font-size:.62rem;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:'+GOLD+';margin-bottom:7px">Análise</div><div style="font-size:.78rem;color:'+DARK+';line-height:1.7">'+e(prec.justificativa)+'</div></div>':'')
      ):(
  ...
  // ══════════════════════════════════════════════════════
  // S8 — ENCERRAMENTO (azul, sem #1d1d1f dominante)
  // ══════════════════════════════════════════════════════
  ...
  slides.push(slide('s8r',
    ...
    // Direita azul com letras brancas
    +'<div style="background:'+BLUE+';padding:56px 64px;display:flex;flex-direction:column;justify-content:center;gap:14px;position:relative;overflow:hidden">'
      ...
      +[
        ...
        return '<div style="background:rgba(255,255,255,.08);padding:14px 16px;display:flex;gap:12px;align-items:flex-start">'
          ...
          +'<div>'
            +'<div style="font-size:.78rem;font-weight:700;color:'+W+';margin-bottom:3px">'+item.title+'</div>'
