...
export default function Historico() {
  ...
  return (
    <div className="space-y-6">
      ...
      {rows.length === 0 ? (
        <div className="card p-12 text-center"><p className="text-gray-400 text-sm">Nenhuma apresentação ainda.</p></div>
      ) : (
        <div className="space-y-2">
          {rows.map(row => (
            <div key={row.id} className="card p-4 flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0">
                <span className="text-blue-600 text-sm font-bold">{(row.residencial||'?')[0].toUpperCase()}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">
                  {row.cliente||'—'}<span className="text-gray-400 font-normal ml-2">· {row.residencial}</span>
                </p>
                <p className="text-xs text-gray-400 mt-0.5">{row.bairro} · {fmt(row.created_at)}</p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => download(row)} className="btn-ghost text-xs">↓ Baixar</button>
                <button onClick={() => del(row.id)} className="text-xs text-gray-300 hover:text-red-400">Excluir</button>
              </div>
            </div>
          ))}
        </div>
