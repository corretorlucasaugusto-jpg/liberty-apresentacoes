...
export default function Gerador() {
  ...
  // ── Auto-save: salva raw_data no Supabase ao sair de qualquer campo ──
  // Cria um rascunho novo se não tiver editId nem draftId ainda
  const autoSave = async () => {
    if (!user?.id) return
    const rawData = collectData()
    // Não salva se o formulário está completamente vazio
    ...
    try {
      if (activeId) {
        ...
        if (inserted?.id) {
          ...
          // Atualiza URL para ?edit=ID sem recarregar
  ...
  // Dispara autoSave com debounce de 600ms após blur em qualquer campo
