...
export default function Login() {
  ...
  const inp = {
    ...
    fontSize: '14px', outline: 'none', background: 'rgba(255,255,255,0.06)',
  ...
  return (
    <div style={{
      minHeight: '100vh',
      background: 'linear-gradient(160deg, #060b14 0%, #0d1829 50%, #060b14 100%)',
      display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center',
      padding: '24px', fontFamily: '-apple-system,BlinkMacSystemFont,system-ui,sans-serif',
    }}>
      {/* Glow effects */}
      <div style={{ position:'fixed', inset:0, overflow:'hidden', pointerEvents:'none' }}>
        <div style={{ position:'absolute', top:'-15%', right:'-5%', width:'500px', height:'500px', borderRadius:'50%', background:'radial-gradient(circle,rgba(18,102,205,0.15) 0%,transparent 70%)' }}/>
        <div style={{ position:'absolute', bottom:'-15%', left:'-5%', width:'400px', height:'400px', borderRadius:'50%', background:'radial-gradient(circle,rgba(212,160,23,0.08) 0%,transparent 70%)' }}/>
      </div>

      {/* Logo — mix-blend-mode removes black JPEG background */}
      <div style={{ textAlign:'center', marginBottom:'8px', position:'relative', zIndex:1 }}>
        <img src={LOGO} alt="Liberty Imóveis"
          style={{
            ...
            filter:'drop-shadow(0 4px 20px rgba(18,102,205,0.4)) brightness(1.1)',
          }}
        />
      </div>
      ...
      <div style={{ textAlign:'center', marginBottom:'32px', position:'relative', zIndex:1 }}>
        <h1 style={{ margin:0, fontSize:'1.5rem', fontWeight:'700', color:'#fff', letterSpacing:'-0.02em' }}>
          Gerador de V2 Liberty
        </h1>
        <p style={{ margin:'6px 0 0', fontSize:'12px', color:'rgba(255,255,255,0.3)', letterSpacing:'0.06em', textTransform:'uppercase' }}>
      ...
      <div style={{
          ...
          {['login','signup'].map(m => (
            <button key={m} onClick={() => { setMode(m); setError(''); setSuccess('') }}
              style={{
                ...
                background: mode===m ? 'rgba(18,102,205,0.7)' : 'transparent',
                color: mode===m ? '#fff' : 'rgba(255,255,255,0.35)',
                boxShadow: mode===m ? '0 2px 8px rgba(18,102,205,0.4)' : 'none',
              }}>
        ...
        <form onSubmit={handle} style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
          <div>
            <label style={{ display:'block', fontSize:'10px', fontWeight:'700', color:'rgba(255,255,255,0.35)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:'7px' }}>E-mail</label>
          ...
          <div>
            <label style={{ display:'block', fontSize:'10px', fontWeight:'700', color:'rgba(255,255,255,0.35)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:'7px' }}>Senha</label>
          ...
          {mode==='signup' && (
            <div>
              <label style={{ display:'block', fontSize:'10px', fontWeight:'700', color:'rgba(255,255,255,0.35)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:'7px' }}>Confirmar senha</label>
          ...
          {error && <div style={{ padding:'10px 14px', borderRadius:'8px', background:'rgba(239,68,68,0.12)', border:'1px solid rgba(239,68,68,0.2)', fontSize:'12px', color:'#fca5a5' }}>{error}</div>}
          ...
          <button type="submit" disabled={loading} style={{
            marginTop:'6px', padding:'13px', border:'none', borderRadius:'10px', color:'#fff',
            fontSize:'14px', fontWeight:'600', cursor: loading ? 'not-allowed':'pointer',
            background: loading ? 'rgba(18,102,205,0.4)':'linear-gradient(135deg,#1266CD,#1a7be8)',
            boxShadow: loading ? 'none':'0 4px 16px rgba(18,102,205,0.45)',
